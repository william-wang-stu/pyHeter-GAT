...
from model import BatchdenseGAT, HeterdenseGAT, HetersparseGAT
import numpy as np
...
class HeterDataset(Dataset):
    def __init__(self, samples: SubGraphSample, args) -> None:
        ...
        if args.model == 'heterdensegat' or args.model == 'hetersparsegat':
    ...
    def extend_hetergraph(self):
        ...
        if self.args.model == 'hetersparsegat':
            self.adjs = np.array([[sparse.csr_matrix(hadj[0]), sparse.csr_matrix(hadj[1])] for hadj in self.adjs])
        ...
        logger.info(f"{self.adjs.shape}, {self.vertex_ids.shape}, {self.labels.shape}, {self.feats.shape}")
    ...
    def add_self_loop(self):
        ...
        if type(self.adjs[0][0]) == np.ndarray:
            ...
            for idx in range(self.adjs.shape[0]):
                self.adjs[idx, 0] += sparse_identity
                self.adjs[idx, 1] += sparse_identity
...
def heter_load_dataset(args):
    ...
    if args.model == 'hetersparsegat':
        samples.adj_matrices = np.array([sparse.csr_matrix(elem) for elem in samples.adj_matrices])
...
def collate_fn2(batch:list):
    ...
    if type(adjs_batch[0][0]) == sparse.csr_matrix:
        adjs_batch = torch.stack([torch.stack([get_sparse_tensor(adj[0].tocoo()), get_sparse_tensor(adj[1].tocoo())]) for adj in adjs_batch])
    else:
...
parser.add_argument('--model', type=str, default='hetersparsegat', help="available options are ['batchdensegat', 'heterdensegat', 'hetersparsegat']")
...
def evaluate(epoch, loader, thr=None, return_best_thr=False, log_desc='valid_'):
    ...
    for i_batch, batch in enumerate(loader):
        graphs, features, labels, vertices = batch
        bs = graphs.size(0)
        if args.cuda:
            graphs, features, labels, vertices = graphs.to(args.gpu), features.to(args.gpu), labels.to(args.gpu), vertices.to(args.gpu)
        ...
        if args.model == 'batchdensegat':
            output = model(vertices, graphs, features, user_feats)
            output = output[:,-1,:]
        elif args.model == 'heterdensegat':
            output = model(vertices, [graphs[:,0], graphs[:,1]], features, user_feats)
            output = output[:,-1,:]
        elif args.model == 'hetersparsegat':
            # assert args.batch == 1
