...
class RNNModel(nn.Module):
    ...
    def __init__(self, rnn_type, user_size, d_word_vec, d_inner_hid, dropout=0.1, tie_weights=False):
    ...
    def forward(self, input, generate=False):
        ...
        if self.use_network:
            outputs = Variable(torch.zeros(max_len, batch_size, self.nhid+self.nhid)).to(emb.device)#
        else:
            outputs = Variable(torch.zeros(max_len, batch_size, self.nhid)).to(emb.device)
        hidden = Variable(torch.zeros(batch_size, self.nhid)).to(emb.device)
        #hidden_c = Variable(torch.zeros(batch_size, self.nhid)).cuda()
        for t in range(0, max_len):
            # GRU
            if self.pos_emb:
                hidden = self.rnn(torch.cat([emb[:,t,:],self.drop(self.pos_embedding(torch.ones(batch_size).long().to(emb.device)*t))],dim=1), hidden)
            else:
