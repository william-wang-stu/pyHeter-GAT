...
class Options(object):
    ...
    def __init__(self, data_name = 'twitter'):
        ...
        self.idx2u_dict = data_name+'/idx2u.data'
        #save path.
        self.save_path = ''
...
def LoadDynamicDiffusionGraph(data_name, time_step_split=time_step_split):
...
def LoadDynamicHeteGraph(data_name):
    ...
    if os.path.exists(options.net_data):
        with open(options.net_data, "rb") as f:
            edges = pickle.load(f)
        follow_relation = [(_u2idx[edge[0]], _u2idx[edge[1]]) for edge in edges if edge[0] in _u2idx and edge[1] in _u2idx]
        # with open(options.net_data, 'r') as handle:
...
class DataConstruct(object):
    ...
    def __init__(
        ...
        self._train_data, _train_data_len = self._readCascadeFromFile2(self.options.train_data, min_user=2, max_user=500)
        self._valid_data, _valid_data_len = self._readCascadeFromFile2(self.options.valid_data, min_user=2, max_user=500)
        self._test_data,  _test_data_len  = self._readCascadeFromFile2(self.options.test_data,  min_user=2, max_user=500)
    ...
    def next(self):
        ...
        if self._iter_count < self._n_batch:
            ...
            seq_users, seq_tss = pad_to_longest2(seq_insts)
