...
from utils import HeterSubGraphSample, load_pickle, save_pickle, init_args, ChunkSampler, HeterGraphDataset, sparse_batch_collate, get_sparse_tensor
from model import BatchGAT, HeterGraphAttentionNetwork, BatchGAT2, HeterGAT2, HeterGAT1
import numpy as np
import time
import torch
...
class DiggDataset(Dataset):
    ...
    def extend_graph(self):
        ...
        if type(self.adjs[0]) == 'numpy.ndarray':
            identity = np.identity(self.adjs.shape[1])
            self.adjs += identity
            self.adjs[self.adjs != 0] = 1.0
            self.adjs = self.adjs.astype(np.dtype('B'))
        elif type(self.adjs[0]) == 'scipy.sparse.csr.csr_matrix':
            sparse_identity = sparse.identity(self.adjs[0].shape[0], format="csr")
            for idx in range(len(self.adjs)):
                self.adjs[idx] += sparse_identity
                self.adjs[idx] = self.adjs[idx].astype(np.dtype('B'))
...
class HeterDataset(Dataset):
    def __init__(self, samples: HeterSubGraphSample) -> None:
        super().__init__()
        self.hadjs = samples.heter_adj_matrices
        self.labels = samples.labels
        self.feats = samples.initial_features
        self.vertex_ids = samples.vertex_ids
        self.extend_graph()
        self.set_dtype()
    def extend_graph(self):
        # self-loop trick, the input graphs should have no self-loop
        if len(self.hadjs.shape)>=2 and type(self.hadjs[0][0]) == sparse.csr.csr_matrix:
            logger.info("Converting sparse matrices to np arrays...")
            hadjs = []
            for idx in range(len(self.hadjs)):
                hadjs.append([adj.toarray() for adj in self.hadjs[idx]])
            self.hadjs = np.array(hadjs, dtype=np.float64)
        if type(self.hadjs[0]) == np.ndarray:
            # identity = np.identity(self.hadjs.shape[1])
            identity = np.identity(self.hadjs.shape[2])
            self.hadjs += identity
            self.hadjs[self.hadjs != 0] = 1.0
            self.hadjs = self.hadjs.astype(np.dtype('B'))
        elif type(self.hadjs[0]) == sparse.csr.csr_matrix:
            sparse_identity = sparse.identity(self.hadjs[0].shape[0], format="csr")
            for idx in range(len(self.hadjs)):
                self.hadjs[idx] += sparse_identity
                self.hadjs[idx] = self.hadjs[idx].astype(np.dtype('B'))
    def set_dtype(self):
        if type(self.hadjs[0]) == np.ndarray:
            self.hadjs  = np.float64(self.hadjs)
        self.feats = np.float32(self.feats)
    def get_feature_dim(self):
        return self.feats.shape[2]
    def __len__(self):
        return self.labels.shape[0]
    def __getitem__(self, index):
        return self.hadjs[index], self.feats[index], self.labels[index], self.vertex_ids[index]

def collate_fn2(batch: list):
    adjs_batch, feats_batch, labels_batch, vertices_batch = zip(*batch)
    if type(adjs_batch[0]) == csr_matrix:
        adjs_batch = torch.stack([get_sparse_tensor(adj.tocoo()) for adj in adjs_batch])
    else:
        adjs_batch = torch.FloatTensor(adjs_batch)
...
def digg_load_dataset(args, train_ratio=60, valid_ratio=20, batch_size=256):
    ...
    # NOTE: 2. Fake SubGraphSample
    # netsize, samplenum = 200, 61801
    # bin_sym_mat = np.random.randint(0, 2, (netsize, netsize))
    # bin_sym_mat^=bin_sym_mat.T
    # samples = SubGraphSample(
    #     adj_matrices=np.transpose(np.repeat(bin_sym_mat[:,:,np.newaxis], samplenum, axis=2), (2,0,1)),
    #     influence_features=np.random.rand(samplenum, netsize, 20),
    #     vertex_ids=np.tile(np.arange(netsize), samplenum).reshape(samplenum,-1),
    #     labels=np.random.randint(0,2, samplenum),
    # )
    # logger.info(f"{samples.adj_matrices.shape}, {samples.influence_features.shape}, {samples.vertex_ids.shape}, {samples.labels.shape}")

    # NOTE: 3. HeterSubGraphSample
...
if args.use_pretrained_emb:
    model = HeterGAT1(n_user=50, pretrained_emb=torch.FloatTensor(embedding), n_units=n_units, n_heads=n_heads, dropout=args.dropout)
...
def evaluate(epoch, loader, thr=None, return_best_thr=False, log_desc='valid_'):
    ...
    for i_batch, batch in enumerate(loader):
        ...
        if not args.use_pretrained_emb:
            ...
            output = model(features, vertices, [graph[:,0], graph[:,1]])
...
def train(epoch, train_loader, valid_loader, test_loader, log_desc='train_'):
    ...
    for i_batch, batch in enumerate(train_loader):
        graph, features, labels, vertices = batch
        # logger.info(f"{graph.shape}, {features.shape}, {labels.shape}, {vertices.shape}")
        bs = graph.size(0)
