...
def load_dataset(args):
    ...
    vertex_ids, indices = np.unique(vertex_ids[:,-1], return_index=True)

    # shuffle
    if args.shuffle:
        batches = [(id, idx) for id,idx in zip(vertex_ids, indices)]
        random.shuffle(batches)
        vertex_ids, indices = zip(*batches)
        vertex_ids, indices = list(vertex_ids), list(indices)

    labels = labels[indices]
    train_start,  valid_start, test_start = 0, int(len(labels) * args.train_ratio / 100), int(len(labels) * (args.train_ratio + args.valid_ratio) / 100)
    # train_ids, valid_ids, test_ids = vertex_ids[train_start:valid_start], vertex_ids[valid_start:test_start], vertex_ids[test_start:]
    # train_labels, valid_labels, test_labels = labels[train_start:valid_start], labels[valid_start:test_start], labels[test_start:]

    n_user = load_pickle(os.path.join(DATA_ROOTPATH, "HeterGAT/basic/deg_le483_subgraph.p")).vcount()
    ...
    return hadjs, feats, feature_dim, torch.LongTensor(labels), vertex_ids, train_start, valid_start, test_start, n_user, nb_classes, class_weight
...
# hadjs, feats, feature_dim, train_labels, valid_labels, test_labels, train_ids, valid_ids, test_ids, n_user, nb_classes, class_weight = load_dataset(args)
hadjs, feats, feature_dim, labels, vertex_ids, train_start, valid_start, test_start, n_user, nb_classes, class_weight = load_dataset(args)
...
logger.info(f"hadjs={hadjs[0].shape}, feats={feats.shape}, n_user={n_user}")
logger.info(f"Length={len(labels[train_start:valid_start])},{len(labels[valid_start:test_start])},{len(labels[test_start:])}, class_weight={class_weight[0]:.2f}:{class_weight[1]:.2f}, nb_classes={nb_classes}, n_units={n_units}, n_heads={n_heads}")
logger.info(f"Train={len(vertex_ids[train_start:valid_start])}, Valid={len(vertex_ids[valid_start:test_start])}, Test={len(vertex_ids[test_start:])}")
...
def evaluate(epoch, thr=None, return_best_thr=False, log_desc='valid_'):
    ...
    if thr is None: # valid
        loss = F.nll_loss(output[vertex_ids[valid_start:test_start]], labels[valid_start:test_start], class_weight)
        y_true  = labels[valid_start:test_start].data.tolist()
        y_pred  = output[vertex_ids[valid_start:test_start]].max(1)[1].data.tolist()
        y_score = output[vertex_ids[valid_start:test_start]][:, 1].data.tolist()
    else: # test
        loss = F.nll_loss(output[vertex_ids[test_start:]], labels[test_start:],  class_weight)
        y_true  = labels[test_start:].data.tolist()
        y_pred  = output[vertex_ids[test_start:]].max(1)[1].data.tolist()
        y_score = output[vertex_ids[test_start:]][:, 1].data.tolist()
...
def train(epoch, log_desc='train_'):
    ...
    loss_train = F.nll_loss(output[vertex_ids[train_start:valid_start]], labels[train_start:valid_start], class_weight)
    loss_train.backward()
    optimizer.step()
